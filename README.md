...
- 연결 인식 모션(`connecting`)의 별 애니메이션 : After Effects `~/Desktop/grad_motion1.aep`
  - 엄지척 구 : `~/Desktop/grad_motion.aep_AME/별_1.mp4` 0.3~3.3s (그 전에는 별_3.mp4 0~2.6s)
  - 스트로크 : `~/Desktop/grad_motion.aep_AME/별_2.mp4` (27s, AE 프로젝트명 별_4)
  - 시안 비교용 옛 버전(노란 그래픽) 엄지척 영상은 `~/Desktop/hospital-robot-gui-shots/old-version_thumbs-up_yellow.mp4` :
  `git archive d7bb4fd^` 로 그 시점 트리를 꺼내 8778 포트로 띄우고 CDP 스크린캐스트(jpeg q95, 1080)로 녹화 → H.264 crf 18
- `star.*` : 별_1.mp4 를 `-ss 0.3` + `scale=2140:1204,crop=1080:1080:536:52` (3.0s)
    - 소스에서 구 실루엣을 적합하면 중심 (965,532) · R 461~470 → R 을 스크린 520 에 맞춰 1.115배, 그 중심을 화면 중앙으로
    - 인코딩 결과 실측 : 절정 1.9s 에 중심 540.4,541.5 · R 521.7. 크롭에서 맞췄으니 CSS 보정은 없다
    - `-ss 0.3` : 구가 가득 차는 시점을 1.7s 로 맞춰 기존 연출(스트로크 1.8s, 언어 전환 3.0s)을 그대로 쓴다
    ...
    - `.stroke{ scale:1.02 }` : 링(R 514)을 살짝 밖으로 밀어 스크린에 걸리는 밝은 띠를 18px → 8px 로 (1.03 은 11px, 1.04 면 거의 사라진다)
...
- 글리밍 마스크는 `::before` 0→22% / `::after` 5→45% (예전 8→55% / 40→100% 는 번짐 무게중심이 글자보다 11px 오른쪽으로 쏠렸다 → 4.6px)
- 엄지척 뒤 문구 그라디언트는 `#fff 0 66% → rgba(255,255,255,.55) 100%` (예전 40%→투명 88.5% 는 글자 아래가 너무 지워져 흰색이 안 느껴졌다)
- 인사 문구 교체는 제자리에서 : `inPlace`(들어올 때) / `out`(나갈 때) 모두 translate 없이 흐림+투명도만, `--i` 로 위→아래·왼→오른쪽 순서
  - **두 겹(`.l-greet` 2개)을 번갈아** 쓴다 : 한 겹만 쓰면 DOM 을 갈아끼우는 순간 앞 문구가 통째로 사라진다.
    앞 문구는 0~1.1s 에 순서대로 사라지고 다음 문구는 0.35s 부터 같은 순서로 올라와 겹치는 순간이 있다 (6s 주기).
    실측 : +0.6s 에 앞 문구 0.06/0.18 · 새 문구 첫 단어 0.09 로 공존
- 언어 전환은 connecting 7.6s, 스트로크(1.8s~)가 차오르는 동안 시작 : 한국어 지우기 2.4~5.4s, 영어 0.9s 뒤따라 3.3~6.3s, 아랫줄 +0.9s
...
- 얼굴이 가까워지면(fill) 구슬이 사라지지 않고 화면을 채우며 커진다(scale 1.2) + `opacity:.045; filter:grayscale(1)`,
  `body.fill .screen::before{ opacity:.12 }` 로 위아래 색 띠도 죽인다 (실측 평균 RGB 16,17,19 — 색 치우침 3 이내. 예전 .1 은 갈빛이 돌았다)
- 카메라 노치는 매끈한 반구(`ellipse cx 540 cy -10 rx 86 ry 88`) : 피그마 원본 path 는 직선 측면 + 모따기가 있어
  베젤을 그리지 않는 이 화면에선 각진 덩어리로 보였다. 아래 꼭지 y 78 은 그대로 (폭 y30 151 → y50 123 → y70 67 → y84 0)

## 편집할 때 조심 (겪은 사고)

- `@keyframes` 블록을 파이썬 인덱스로 자를 때 `s.index('}', ...)` 는 **첫 키프레임의 닫는 괄호**에서 멈춘다.
  두 번 그렇게 잘라 떠도는 조각이 남았고, 그 뒤 규칙(`body.connecting .star{opacity:1}`)이 무효화돼
  엄지척 구가 화면에서 아예 안 보였다 (커밋 f516791·3d78aa7). 블록 전체를 문자열로 정확히 지정해 지울 것
- 확인법 : `<style>` 의 중괄호 균형 + CSSOM 에서 선택자·키프레임 존재 확인 (지금 199개 규칙, 누락 없음)
