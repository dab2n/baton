...
- 연결 인식 모션(`connecting`)의 별 애니메이션 : After Effects `~/Desktop/grad_motion1.aep`
  ...
  - `star.*` : 0~2.6s 를 `scale=2248:1264,crop=1080:1080:602:94` (절정 글로우 약 888 → 1034, 스크린 지름 1040 을 거의 채움. 실측 중심 541,541)
    - 구 중심은 시간에 따라 움직인다. 프레임별로 방향별 도달 반지름을 재 `r(a) = R + dx·cos a + dy·sin a` 로 적합하면
      1.70s R 503 중심 왼쪽 11.7·위 10.2 (오른쪽 띠 34px) / 1.80s R 510 중심 2.4 / 1.90s~ R 521~531 중심 ±11 (이미 가운데, 가장자리를 넘김)
      → `@keyframes starFit` (영상과 같은 2.6s) 로 1.7~1.8s 만 보정하고 1.95s 부터는 원본 그대로.
      검은 띠 1.70s 18.6 → 0.8px(최대 34 → 2px), 1.80s 10.5 → 4.2px. 절정까지 보정하면 0.1s 에 13px 씩 흔들려 더 어색하다
    - 페이지 렌더로는 배경 구·베젤이 가장자리를 밝혀 이 띠를 못 재니 영상 프레임 픽셀로 잰다
...
- 자간 : `.device *{ letter-spacing:0.015em }` (예전 −0.01em 이 너무 좁았다). 대체 폰트 보정값도 이 기준에 측정 델타를 더한 값
