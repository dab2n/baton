...
- 언어 전환은 connecting 7.2s, 스트로크가 뜨는 순간에 맞춘다 : 스트로크 1.7s 시작(페이드 0.4s), 한국어 지우기 1.9~4.9s, 영어 0.9s 뒤따라 2.8~5.8s, 아랫줄 +0.9s
  - 실측 : 1.95s 에 스트로크 opacity 0.14 → 2.3s 에 1.00, 같은 2.3s 에 한국어 마스크가 85 → 84.4% 로 움직이기 시작
- 강조어 글리밍은 글자와 같은 시각·같은 길이(0.9s)로 떠오른다 (실측 : 0.7s 에 글자 0.40 · 겹 0.40, 1.0s 에 0.90 · 0.90).
