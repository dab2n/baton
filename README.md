...
- Hello! Leo (예전 Somi) : 피그마 `node-id=3139-207`
...
- 인사 문구 교체는 제자리에서 : `inPlace`(들어올 때) / `out`(나갈 때) 모두 translate 없이 흐림+투명도만, `--i` 로 위→아래·왼→오른쪽 순서
  - **두 겹(`.l-greet` 2개)을 번갈아** 쓴다 : 한 겹만 쓰면 DOM 을 갈아끼우는 순간 앞 문구가 통째로 사라진다.
    앞 문구는 1.1s 에 걸쳐(단어 간격 90ms) 순서대로 사라지고, 다음 문구는 0.5s 부터 1.3s 에 걸쳐(간격 100ms) 같은 순서로 올라온다 (6.5s 주기).
    실측 : 앞 문구 0.53/0.66 일 때 새 문구가 뜨기 시작해 0.18/0.29 대 0.34/0.17/0.05 로 한동안 겹친다
- 언어 전환은 connecting 5.6s : 스트로크 0.9s 시작(구가 가득 찬 뒤), 한국어 지우기 1.1~3.6s, 영어 0.6s 뒤따라 1.7~4.4s, 아랫줄 +0.6s
  - 시작은 이르게(엄지척 직후 흐름) 두고 드러나는 속도만 천천히 : 이름(Leo)이 서서히 드러나게
  - 실측 : 1.5s 한국어 마스크 81.7% → 4.0s 15%, 영어는 4.6s 에 완료
- 강조어 글리밍은 글자와 같은 시각에 시작해 **왼→오로 훑고 지나간다** : 마스크 두 겹(번짐 + 와이프)의 교집합으로,
  와이프 폭을 0 → 140% 로 애니메이션한다 (실측 0.3s 0% → 0.6s 28%(첫 글자 0.20) → 0.9s 140%).
  줄 글리밍(언어 전환)은 전환 마스크가 이미 왼→오로 지나가므로 와이프는 끝까지 열어 둔다.
  예전에는 글자가 다 나타난 뒤 1.0s 에 켜져 나중에 얹힌 느낌이었다
...
- 길안내 게이지는 스크린 **아래 정중앙(540,1015 · 반지름 475 원의 90°)** 에서 출발해 오른쪽 목적지 핀까지 (예전엔 좌하단 139° 에서 시작)

- 카드(`.rec`)는 DOM 이 스트로크보다 뒤라 그냥 두면 가장자리 그래픽 위를 지나간다 → `.stroke{ z-index:3 }` 로 올려 카드가 뒤로 사라지게
- 스크린 위아래 색 띠는 **스크린과 같은 중심(540,540)의 동심원**으로 번지게 하고 위/아래는 아주 긴 마스크로만 나눈다
  (`.screen::before` 감청 · `.screen::after` 밝은 회색). 직선 그라디언트는 검은 화면에서 경계가 가로줄로 보였다.
  실측 : 띠 경계가 가운데보다 옆에서 130~180px 더 깊고, 이웃 행 밝기 변화는 평균 1 이하(계단 없음).
  번짐은 테두리에서 약 150~200px 안쪽까지만 (더 길게 풀면 블러가 티나고 가운데 검정이 침범당한다)
- 아래 띠는 **차가운 회색 #2B313C** : 중성 회색(#4A4A4A)은 주변 남색과 대비돼 노랗게 읽혔다.
  위 띠도 #16213A 로 낮췄다 — 검은 스크린의 어두운 명도를 지키는 선까지만 (실측 위 rgb(6,8,15) / 아래 rgb(33,38,47))
- **기본 화면(idle)은 위아래 아무것도 없이 순검정** (실측 rgb(0,0,0)). 띠는 얼굴이 인식돼 구슬이 커질 때부터
  `transition:opacity 1.4s` 로 스며 나온다 (실측 0.09 → 0.84 로 1.3s 에 걸쳐 번진다)
- 구슬은 idle 에서 배회한 **그 자리에서 이어서** 커진다 : 배회 애니메이션을 끊으면 translate 가 0 으로 튀므로,
  go() 에서 현재 값을 인라인으로 물려 class 를 바꾸고 다음 프레임에 지운다 (실측 −46,−30 → 0 으로 1.2s 에 이어짐)
- 게이지는 트랙 알파 .07 · 채움 30px 로 흰색이 도드라지게 (실측 채움 235 vs 트랙 19)
- 길안내 장면에선 아래 띠를 opacity .45 로 낮춘다 : 게이지와 밝기가 비슷해 게이지가 뒤 레이어처럼 보였다
  (실측 게이지 196 vs 안쪽 배경 11 → 대비 185)
- 띠는 `.screen` 의 가상 요소라 항상 **가장 뒷 레이어** : 콘텐츠·영상·게이지는 모두 그 위에 온다
- 구슬이 커지는 fill 장면에서도 띠는 보인다 (opacity .8 / 발광 .5). 예전엔 .12 로 꺼서 그냥 검은 화면이었다
- 그 색은 `.rimlight` (원뿔 그라디언트를 테두리 쪽만 남긴 겹)가 18s 에 한 바퀴 돌아 가장자리를 따라 일렁인다
...
- 스트로크 블러 : 영상은 한 겹이라 선명/흐림을 동시에 못 준다 → 링 위에 `backdrop-filter` 고리 세 겹(`.sb1~3`, 3/7/12px)을
  **형제 요소로** 얹는다 (한 요소의 가상 요소로 넣으면 부모 mask 가 backdrop root 가 되어 뒤를 못 읽는다).
  강한 겹일수록 밝은 선 안쪽 가장자리 바로 앞까지 와서 빛을 안쪽으로 번지게 하고, 바깥 끝(r 516~)은 선명.
  실측 반지름별 밝기(끔→켬) : r496 18→72, r500 21→85, r504 58→106, r512~519 그대로. 블러 켠 장면도 60fps
- 기록·재진 장면 : **카드 없이 문장만** (카드 이미지 파일은 저장소에 남겨 둠). 38px.
  첫 문장은 화면 가운데 한 줄, 다음 문장이 나오면 첫 문장이 위로 34.2px 올라가 두 줄이 함께 가운데 (실측 중심 505 / 575)
- 등장 리듬은 **문장 단위** : 한 문장 안에서는 글자(띄어쓰기 한 칸 포함)가 30ms 간격으로 끊김 없이 흘러나오고
  (등장 0.95s 라 여러 글자가 겹쳐 번져 딱딱 끊기지 않는다), 쉬는 곳은 문장 끝 +450ms 와 한국어 문구의 줄바꿈 +380ms 뿐.
  단어마다 말하는 시간만큼 뜸을 들이는 방식은 어색해서 버렸다. 문장별 속도 rate : 영어 인사 1.8, 길안내 라벨 .8.
  일반 글자 번짐은 알파 ×.35 로 옅게, 강조어만 짙게
- 두 줄(기록 + 재진) 이 다음 장면으로 넘어가며 사라질 때도 윗줄 위치(−34.2px)를 유지한다 (`body.guide .l-msg`) :
  없으면 translate 가 0 으로 튀어 두 줄 간격이 갑자기 좁아진 채 흐려졌다
- **모든 문장은 왼쪽부터 한 글자씩** (한국어 문구 · Good Afternoon! Leo. · 기록·재진·안내 · 길안내 라벨) — `letters()` 가
  단어(.w) 안의 글자를 인라인 `.c` 로 쪼갠다. 글자 색·그라디언트도 `.c` 에 입힌다 : 단어에 입히면 background-clip:text 가
  아직 안 켜진 글자까지 그려 단어가 통째로 먼저 보인다 (그래서 강조어가 아닌 앞부분이 뚝 나와 있었다).
  영어 인사는 지우개 마스크 대신 1.7s 부터 글자당 110ms 로, 한국어 문구 교체는 앞 문구가 한 글자씩 사라지는 동안 다음 문구가 한 글자씩
- 강조어(em)는 켜진 뒤에도 비눗방울 번짐이 글자를 따라 물결처럼 일렁인다 (`emGlow`, 글자마다 90ms 씩 늦게 시작)
- 영어 안내 문장(기록·재진·안내)은 **알파벳 단위**로 나타나고 가장 최근 글자에 유리·비눗방울 색 번짐(`letterIn`) :
  엄지척 구의 흰 핵·연두·파랑·하늘에서 두 색 짝(--g1/--g2)을 글자마다 돌려 쓰고, 번짐은 글자가 켜질 때 가장 밝다가
  뒤따르는 글자 몇 개가 지나는 동안 사라진다. 글자는 **인라인** span 으로 나눈다 (inline-block 이면 커닝이 끊기고 글리밍 겹과 어긋남).
  강조어 글리밍은 그 강조어의 첫 글자(--k0)에 시작해 글자 수(--kn)만큼 같은 속도로 훑는다
- 스트로크 안쪽 블러 일렁임 : 고리 마스크에 원뿔 세기 분포를 교집합으로 겹치고 `@property --sa` 각도를 겹마다
  26s / 19s(역방향) / 31s 로 돌린다 (실측 5초 사이 각도별 안쪽 밝기 45→64, 66→43 등으로 흐른다. 60fps)
...
- 전역 스크립트가 한 덩어리라 `let` 이름이 겹치면 **파일 전체가 SyntaxError** 로 죽는다 (문구 겹 인덱스를 `shown` 으로
  두었다가 길안내 숫자의 `shown` 과 충돌 → 장면이 아무것도 안 움직였다). CDP `Runtime.exceptionThrown` 으로 바로 잡힌다
