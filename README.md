...
- 연결 인식 모션(`connecting`)의 별 애니메이션 : After Effects `~/Desktop/grad_motion1.aep`
  ...
  - `star.*` : 0~2.6s 를 `scale=2248:1264,crop=1080:1080:602:94` (절정 글로우 약 888 → 1034, 스크린 지름 1040 을 거의 채움. 실측 중심 541,541)
    - 구가 다 자란 뒤에도 0.6s 동안 반지름이 500 → 535 로 맥동한다. 검은 띠가 보이는 1.65~1.80s 프레임에서
      방향별 도달 반지름을 재 `r(a) = R + dx·cos a + dy·sin a` 로 적합하면 R 505, 중심이 왼쪽 8.4 · 위 6.8
      (오른쪽 띠 34px, 왼쪽 1px) → CSS 에서 `translate:8.7px 7px; scale:1.029` 로 중심을 맞추고 520 에 닿게 한다.
      검은 띠 평균 19 → 4px, 최대 34 → 9px. 페이지 렌더로는 배경 구·베젤이 가장자리를 밝혀 이 띠를 못 재니 영상 프레임으로 잰다
